...
from chemistry.functions import create_wrapper, WrapperConfig
...
# объявление класса и заполнение списка координат
molecule, x = Molecule.load_chemcraft_struct('input.xyz', create_wrapper(WrapperConfig.default()))

# количество атомов в  молекуле
N = int(molecule.n_dims / 3)
print(N)

# матрица с исходными координатами
coordinate_atoms = np.array(x).reshape((N, 3))
...
for i in range(N):   # рассматриваем каждый атом
    ...
    # узнаем массу атома из словаря и заполняем матрицу масс
    m = mass_atoms.get(f.read(2).replace(" ", ""))
    mass[i] = m
    print("здесь масса")
    print(m)
...
print(len(W))
A = np.matmul(np.matmul(np.transpose(W), molecule.hess(x)), W)
print(A)

# получаем диагональную матрицу
_, L = np.linalg.eig(A)
X_end = np.matmul(np.matmul(np.transpose(L), A), L)
print(X_end)
